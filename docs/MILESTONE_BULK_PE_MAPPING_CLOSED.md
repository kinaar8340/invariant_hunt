# Milestone: Pre-merger bulk PE-power mapping — **fully closed (v1–v4)**

**Status:** closed / frozen — **archived**  
**Date:** 2026-07-10  
**Decision:** Stop the bulk PE-power (and remnant-mass) mapping stretch.  
...
(not a mild power rescue of \(M_{\mathrm{tot}}\), SNR, \(d_L\), or \(M_f\)).

## Closed family (executed)

| Campaign | Variable | Verdict | Status |
|----------|----------|---------|--------|
| **v1** | Event-independent α (universal band) | **Demoted** (0 SUCCESS on true held-outs) | Closed |
| **v2** | Total mass \(M_{\mathrm{tot}}^p\), \(p=1\) | **FALSIFY** (\(z\approx 30\)) | Closed |
| **v3a** | Inverse network SNR \(q=1\) | **FALSIFY** (\(z\approx 21\)) | Closed |
| **v3b** | Luminosity distance \(s=1\) | **FALSIFY** (\(z\approx 18.5\)) | Closed |
| **v4** | Remnant mass \(M_f^p\), \(p=1\) | **FALSIFY** (\(z\approx 30.6\); wrong-way scale ~0.89) | Closed |
...
**Family verdict:** All pre-registered mild bulk-PE / remnant-mass unifications of
\(\alpha_0\) between core and GW170809 **failed**. Stretch **fully closed**.
...
- Free exponents after seeing held-out \(\hat\alpha\)  
...
# Not allowed without new PREREG: mapping v5+ bulk PE / remnant power rescue
```
...
| Variable | **Not** \(M_{\mathrm{tot}}\) / inv-SNR / \(d_L\) / \(M_f\) mild powers already closed |
...
- Not a claim that no map exists — only that **these bulk PE / remnant families failed**  
