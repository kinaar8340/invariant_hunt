...
PHI_CONJ = (1.0 + math.sqrt(5.0)) / 2.0
GOLDEN_ANGLE_DEG = 360.0 * (1.0 - 1.0 / PHI_CONJ)  # ≈ 137.5078°
GOLDEN_ANGLE_RAD = math.radians(GOLDEN_ANGLE_DEG)
PHI_INV2 = 1.0 / PHI_CONJ**2
...
class RubikConeConduit(TwistedHelicalConduit):
    ...
    def steps_for_lambda_t(self, lambda_t_target: float = 2.0, dt: float = 1.0) -> int:
        ...
        return max(1, int(round(lambda_t_target / (self.effective_decay_rate() * dt))))
    ...
    def run_survival_probe(
        ...
        See relaxation_survival.py for the exponential-decay mapping.
        """
